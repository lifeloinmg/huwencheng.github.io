...
#  一、单例模式
表示全局唯一    处理资源访问冲突
问题：OOP 的三大特性是封装、继承、多态；单例将构造私有化不会有继承多态；不方便进行横向扩展
## 1、实现方式
### 1.1、饿汉式
在类加载的时候初始化静态变量
```
...
```
### 2.2、序列化与反序列化安全
反序列化创建新对象，不会调用任何构造方法,直接从流里恢复字段，绕开了 Java 的对象创建正常流程,私有构造器、你的 instance 判断，对反序列化完全无效;
readResolve是反序列化后的 “替换钩子”，反序列化完新对象后，丢掉这个新对象，直接返回单例对象。
```
    // 需要加这么一个方法
