...
<img width="1079" height="806" alt="Image" src="https://github.com/user-attachments/assets/273ea687-6931-4e2f-befa-ae396b5835c0" />

### 1.2、工厂方法

根据不同的参数选择不同的工厂，不同的工厂创建不同的产品

<img width="1490" height="1112" alt="Image" src="https://github.com/user-attachments/assets/be4edf91-47a3-44b2-ade2-f745139003f2" />

### 1.3、抽象工厂

根据不同的参数选择不同的工厂，工厂创建不同的产品族

<img width="1690" height="1680" alt="Image" src="https://github.com/user-attachments/assets/52e744c9-4046-4901-aacc-b81141830f1b" />
...
抽象创建者（Abstract Builder）—— 构建产品的“接口规范”

具体创建者（Concrete Builder）—— 执行构建的“实际操作者”

指挥者（Director）—— 控制构建流程的“总指挥”
...
注解在编译过程之中会自动创建一个静态内部类【类名+Build】对象进行构建
...
builder = builder.id(1L);
builder = builder.name("小明");
builder = builder.age(20);
builder = builder.email("test@qq.com");
...
```
User.builder()       // 拿到 builder
     .id(1L)         // builder赋值，返回自己
     .name("小明")   // 继续赋值，返回自己
     .age(20)        // 继续赋值，返回自己
     .build();       // 最后生成对象
```
...
实现Cloneable接口，并且重写clone方法（来自object类的方法）
